        ...
        <footer className="mx-3 sm:mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-8 sm:gap-10 py-8 sm:py-10 border-b border-gray-200 text-gray-600">
                    <div>
                        <Link to="/" className="text-4xl font-semibold text-gray-800">
                            <span className="text-blue-600">Ziz</span>la<span className="text-blue-600 text-5xl leading-0">.</span>
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm">{t('welcome')}, {t('description')}</p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <a href={item.link} key={i} target="_blank" rel="noreferrer" className={`group flex items-center justify-center w-10 h-10 bg-gray-50 border border-gray-200 hover:scale-110 hover:border-transparent transition-all duration-300 rounded-full ${item.hoverColor}`}>
